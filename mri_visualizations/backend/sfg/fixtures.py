...
def ensure_fixtures() -> None:
    ...
    src = next(iter(sorted(config.ADNI_DIR.glob("*_T1.nii.gz"))), None)
    if src is None:
        src = next(iter(sorted(config.IXI_DIR.glob("IXI*-Guys-*-T1.nii.gz"))), None)
    if src is None:
        return  # no base scan available
    stem = src.name[: -len(".nii.gz")]
    base_site = ("ADNI-" + src.name.split("_")[0]) if src.parent == config.ADNI_DIR else "IXI/Guy's"
    jobs = [
